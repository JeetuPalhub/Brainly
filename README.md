...
- AI chat ("Ask Your Brain") - a RAG assistant that answers questions grounded in your notes, generated by Groq (Llama 3.3)
...
| AI | Groq (Llama 3.3 chat) + HuggingFace Inference API (BART, BGE embeddings) |
| Auth | JWT, bcrypt |
...
Express REST API (TypeScript)
   ...
   |- rate limiter + security headers (middleware)
        |                          |
        v                          v
   MongoDB (Mongoose)        AI providers
    - users, content,         - Groq (Llama) ...... chat generation
      tags, collections,      - HuggingFace ....... tagging, summary, embeddings
      links, ai_cache         - local fallback .... deterministic, always available
```
...
- **RAG chat** (Retrieval-Augmented Generation) answers questions about *your* notes: it embeds the question, retrieves the most similar saved items, feeds them to an LLM as context, and generates an answer grounded in what you saved.
- **Provider fallback chain** keeps the chat working no matter what: it tries **Groq** (fast Llama models) first, falls back to **HuggingFace**, and if both are unavailable returns an **extractive answer** built from the retrieved notes - so it degrades gracefully instead of erroring.
...
- **Graceful fallback** for tagging/embeddings: if HuggingFace is down, a deterministic local method takes over so the app keeps working; every result is labelled `huggingface`, `groq`, or `fallback`.
